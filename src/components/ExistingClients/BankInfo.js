...
import { existingCustomer } from "../../apis/existingcustomer";
import styles from "./ExistingClient.module.css";

const BankInfo = ({ setFormNumber }) => {
  const {
    register,
    handleSubmit,
    watch,
  ...
  const [irsLetterFile, setIrsLetterFile] = useState([]);
  ...
  const handleFileEvent = async (e) => {
    ...
    setIrsLetterFile([...irsLetterFile, response.link]);
  ...
  const handleOnSubmit = async (body) => {
    try {
      body.irs_letter_file = irsLetterFile;
      body.tax_documents = taxDocLinks;
      console.log(body);
      const personalData = JSON.parse(localStorage.getItem("personalData"));
      const irsQuestions = JSON.parse(localStorage.getItem("irsQuestions"));
      const res = await existingCustomer({
        ...body,
        ...personalData,
        ...irsQuestions,
      });
            ...
            >
              ...
              <div className="row">
                <div className="col-lg-6 col-md-6 col-sm-12">
                  ...
                  <select
                    {...register("tax_consultant", { required: false })}
                    id="inlineFormCustomSelect"
                  >
